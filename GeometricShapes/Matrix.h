...
public:
  ...
  TMatrix(const TVector<TVector<T>>& p);
  ~TMatrix();
  ...
  virtual int findCol(T p);
  virtual int** Find(T p);
};
...
template<class T>
inline TMatrix<T>::TMatrix(const TVector<TVector<T>>& p) :TVector<TVector<T>>(p)
{
}
...
template<class T>
inline int TMatrix<T>::findCol(T p)
{
  int col = 0;
  for (int i = 0; i < width; i++)
    for (int j = 0; j < this->len; j++)
      if (p == this->data[i][j]) col++;
  return col;
}

template<class T>
inline int** TMatrix<T>::Find(T p)
{
  int f = 0;
  int** mas = new int*[2];
  for (int k = 0; k < 2; k++) mas[k] = new int[findCol(p)];
  for (int i = 0; i < width; i++)
    for (int j = 0; j < this->len; j++)
      if (p == this->data[i][j]) {
        mas[0][f] = i;
